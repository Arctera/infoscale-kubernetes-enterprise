...
check_split_brain() {
    ...
    # SCSI-3 PR node registrations only exist on disks that belong to THIS
    # InfoScale cluster's diskgroups, identified by the cluster id:
    #   - shared kube data diskgroup : vrts_kube_dg-<clusterid>  (CVM-imported on all nodes)
    #   - fencing/coordinator group  : vrts_coord_dg-<clusterid> (imported on ONE node only)
    # The cluster id is taken from the SDS pod name (infoscale-sds-<clusterid>-...).
    # Plain 'vxdisk list' prints the GROUP name only where a diskgroup is imported
    # on this node, so matching these two names covers the data DG on every node
    # and the fencing DG on the single node that imports it. Free/'online invalid'
    # disks and disks in any other/foreign diskgroup (vxfendg, dg1, dg2, deported
    # groups shown as '-') are excluded, preventing false split-brain reports.
    local cluster_id kube_dg coord_dg
    cluster_id="$(sed -n 's/^infoscale-sds-\([0-9][0-9]*\)-.*/\1/p' <<<"$pod")"

    if [[ -n "$cluster_id" ]]; then
        kube_dg="vrts_kube_dg-${cluster_id}"
        coord_dg="vrts_coord_dg-${cluster_id}"
        unique_disks="$(echo "$disk_info" | awk -v kd="$kube_dg" -v cd="$coord_dg" '
            NR > 1 && $0 ~ /online[[:space:]]+invalid/ { next }
            NR > 1 && ($4 == kd || $4 == cd) { print $1 }
        ' | sort -u)"
    else
        # Fallback: cluster id not derivable from pod name; match the cluster
        # diskgroup name prefixes instead.
        kube_dg="vrts_kube_dg-*"; coord_dg="vrts_coord_dg-*"
        unique_disks="$(echo "$disk_info" | awk '
            NR > 1 && $0 ~ /online[[:space:]]+invalid/ { next }
            NR > 1 && $4 ~ /^vrts_(kube|coord)_dg-/ { print $1 }
        ' | sort -u)"
    fi

    if [[ -z "$unique_disks" ]]; then
        log_info "[${RULE_NAME}] No member disks of ${kube_dg} or ${coord_dg} imported in pod $pod; split brain check skipped"
        return 0
    fi

    log_info "[${RULE_NAME}] Split brain check scoped to ${kube_dg}/${coord_dg} member disk(s) in pod $pod: $(echo $unique_disks | tr '\n' ' ')"
