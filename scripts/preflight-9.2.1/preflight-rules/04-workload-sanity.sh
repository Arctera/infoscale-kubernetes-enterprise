...
check_infoscale_vms_cached() {
        ...
        # Identify powered-off VMs (printableStatus=Stopped, or spec.running=false,
        # or spec.runStrategy=Halted). These are skipped for RWX/LiveMigratable
        # validation: LiveMigratable is a VMI condition that exists only while a VM
        # is running, so a powered-off VM (no VMI) would otherwise be FALSELY
        # flagged as not-LiveMigratable, and a stopped VM does not live-migrate
        # during node drain.
        local -A powered_off_vm=() powered_off_logged=()
        while IFS=$'\t' read -r poff_ns poff_vm; do
                [[ -z "$poff_vm" ]] && continue
                powered_off_vm["${poff_ns}/${poff_vm}"]=1
        done < <(
                jq -r '
                  .items[]
                  | ((.status.printableStatus // "") | ascii_downcase) as $st
                  | (.spec.running) as $run
                  | ((.spec.runStrategy // "") | ascii_downcase) as $rs
                  | select($st == "stopped" or $run == false or $rs == "halted")
        ...
        while read -r ns vm vtype vol; do
                ...
                if [[ -n "${powered_off_vm["${ns}/${vm}"]:-}" ]]; then
                        if [[ -z "${powered_off_logged["${ns}/${vm}"]:-}" ]]; then
                                log_info "[${RULE_NAME}] VM ${ns}/${vm} is powered off; skipping RWX/LiveMigratable check"
                                powered_off_logged["${ns}/${vm}"]=1
                        fi
                        continue
                fi
