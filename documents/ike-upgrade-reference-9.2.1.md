...
> **Note:** Prior to upgrading InfoScale to version 9.2.1, ensure that the license secret has been created. Refer to the official *InfoScale for Kubernetes 9.2.1 Administrator's Guide* for steps.

If the license secret has not already been created, create it before starting the upgrade:

```bash
oc create secret generic infoscale-license -n infoscale-vtas \
  --from-file=infoscale-license.lic=<license_file_location> \
...
For installation instructions, full command reference, example output, and guidance on interpreting results, see [Preflight guide for InfoScale upgrade and fresh install](https://github.com/Arctera/infoscale-kubernetes-enterprise/blob/IKE-9.2.1/documents/preflight-guide-9.2.1.md).
...
1. Verify that the kubelet configuration is applied and the rollout is complete before starting the software upgrade. Refer to the [Enabling kubelet inhibitor with systemd](https://github.com/Arctera/infoscale-kubernetes-enterprise/blob/IKE-9.2.1/documents/Enabling-kubelet-inhibitor-9.2.1.md) section.
...
> **Note:** Kubelet inhibitor configuration is a prerequisite for all VIKE releases. Ensure it is enabled before starting the software upgrade. Refer to the *Prerequisites* section of the *InfoScale for Kubernetes 9.2.1 Administrator's Guide* for steps to enable kubelet inhibitors.
...
#### Patch InfoScale cluster version to 9.2.1

```bash
oc patch infoscalecluster <cluster-name> -n <namespace> \
  --type=merge -p '{"spec":{"version":"9.2.1"}}'
```
...
oc patch infoscalecluster <cluster-name> -n infoscale-vtas \
  --type=merge -p \
  '{"metadata":{"annotations":{"infoscale.veritas.com/combine-upgrade":"enabled"}},"spec":{"version":"9.2.1"}}'
```
